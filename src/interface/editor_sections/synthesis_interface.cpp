...
SynthesisInterface::SynthesisInterface(
                                       ...
                                       const vial::output_map& poly_modulations) : SynthSection("synthesis") {
...
void SynthesisInterface::resized() {
  ...
  int sample_y = oscillators_[vial::kNumOscillators - 1]->getBottom() + padding;
...
void SynthesisInterface::distortionTypeChanged(OscillatorSection* section, int distortion_type) {
  bool dependents[vial::kNumOscillators];
  for (int i = 0; i < vial::kNumOscillators; ++i)
    dependents[i] = false;
  ...
  while (!dependents[index]) {
    ...
    if (vial::SynthOscillator::isFirstModulation(type))
      index = vial::ProducersModule::getFirstModulationIndex(index);
    else if (vial::SynthOscillator::isSecondModulation(type))
      index = vial::ProducersModule::getSecondModulationIndex(index);
    else
      return;
  }
...
void SynthesisInterface::oscillatorDestinationChanged(OscillatorSection* section, int destination) {
  bool filter1_on = destination == vial::constants::kFilter1 || destination == vial::constants::kDualFilters;
  bool filter2_on = destination == vial::constants::kFilter2 || destination == vial::constants::kDualFilters;
  for (int i = 0; i < vial::kNumOscillators; ++i) {
