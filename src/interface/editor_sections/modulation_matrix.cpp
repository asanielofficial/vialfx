  ...
  PopupItems createSubMenuForParameterPrefix(const std::string& name, const String& prefix,
    ...
    for (const String& parameter_name : parameter_names) {
      if (parameter_name.substring(0, prefix_length) == prefix) {
        ...
        if (local)
          display_name = vial::Parameters::getDetails(parameter_name.toStdString()).local_description;
        else
          display_name = vial::Parameters::getDetails(parameter_name.toStdString()).display_name;
  ...
  PopupItems createMiscSubMenu(const std::string& name, const std::vector<String>& parameter_names) {
    ...
    for (const String& parameter_name : parameter_names) {
      ...
      if (!prefix_match && parameter_name != String(kNoConnectionString)) {
        std::string display_name = vial::Parameters::getDetails(parameter_name.toStdString()).display_name;
        items.addItem(index, display_name);
      }
      index++;
    }
...
class ModulationMeterReadouts : public BarRenderer {
  public:
    ModulationMeterReadouts() : BarRenderer(vial::kMaxModulationConnections, false),
    ...
    void updatePositions(int index) {
      ...
      setBarWidth(vial::kMaxModulationConnections * modulation_bounds_[0].getHeight() / height);
      ...
      for (int i = 0; i < vial::kMaxModulationConnections; ++i) {
        if (modulation_active_[i]) {
          ...
          if (value == vial::StatusOutput::kClearValue)
            value = 0.0f;
          float t = vial::utils::clamp(0.5f * (value + 1.0f), 0.0f, 1.0f);
          float x = vial::utils::interpolate(min_x, max_x, t);
  ...
  private:
    ...
    const vial::StatusOutput* modulation_amounts_[vial::kMaxModulationConnections];
    Rectangle<int> modulation_bounds_[vial::kMaxModulationConnections];
    int scroll_offset_;
    bool modulation_active_[vial::kMaxModulationConnections];
};
...
void ModulationMatrixRow::updateDisplayValue() {
  ...
  if (last_destination_value_ > 0.0 && last_source_value_) {
    vial::ValueDetails details = vial::Parameters::getDetails(connection_->destination_name);
...
ModulationMatrix::ModulationMatrix(const vial::output_map& sources, const vial::output_map& destinations) :
...
void ModulationMatrix::setAllValues(vial::control_map& controls) {
...
void ModulationMatrix::checkNumModulationsShown() {
  if (row_order_.size() != vial::kMaxModulationConnections)
    return;
  ...
  num_show = std::min(vial::kMaxModulationConnections, num_show);
