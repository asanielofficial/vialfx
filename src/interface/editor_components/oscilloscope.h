...
class Oscilloscope : public OpenGlLineRenderer {
  public:
    ...
    void setOscilloscopeMemory(const vial::poly_float* memory) { memory_ = memory; }

  private:
    const vial::poly_float* memory_;
...
class Spectrogram : public OpenGlLineRenderer {
  public:
    ...
    void drawWaveform(OpenGlWrapper& open_gl, int index);
    void render(OpenGlWrapper& open_gl, bool animate) override;
    void setAudioMemory(const vial::StereoMemory* memory) { memory_ = memory; }
  ...
  private:
    ...
    const vial::StereoMemory* memory_;
