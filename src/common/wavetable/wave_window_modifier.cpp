...
float WaveWindowModifier::applyWindow(WindowShape window_shape, float t) {
  if (window_shape == kCos)
    return 0.5f - 0.5f * cosf(vial::kPi * t);
  if (window_shape == kHalfSin)
    return sinf(vial::kPi * t / 2.0f);
  if (window_shape == kSquare)
    return t < 1.0f ? 0.0f : 1.0f;
  if (window_shape == kWiggle)
    return t * cosf(vial::kPi * (t * 1.5f + 0.5f));
  return t;
}
...
void WaveWindowModifier::WaveWindowModifierKeyframe::render(vial::WaveFrame* wave_frame) {
  ...
  for (int i = vial::WaveFrame::kWaveformSize; i >= 0; --i) {
    float t = i / (vial::WaveFrame::kWaveformSize - 1.0f);
...
void WaveWindowModifier::render(vial::WaveFrame* wave_frame, float position) {
