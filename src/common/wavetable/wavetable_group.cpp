...
void WavetableGroup::render(vial::WaveFrame* wave_frame, float position) const {
...
void WavetableGroup::renderTo(vial::Wavetable* wavetable) {
  for (int i = 0; i < vial::kNumOscillatorWaveFrames; ++i) {
...
void WavetableGroup::loadDefaultGroup() {
  ...
  vial::WaveFrame* wave_frame = wave_source->getWaveFrame(0);
  for (int i = 0; i < vial::WaveFrame::kWaveformSize; ++i) {
    float t = i / (vial::WaveFrame::kWaveformSize - 1.0f);
    int half_shift = (i + vial::WaveFrame::kWaveformSize / 2) % vial::WaveFrame::kWaveformSize;
