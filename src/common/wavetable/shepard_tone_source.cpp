...
void ShepardToneSource::render(vial::WaveFrame* wave_frame, float position) {
  if (numFrames() == 0)
    return;

  WaveSourceKeyframe* keyframe = getKeyframe(0);
  vial::WaveFrame* key_wave_frame = keyframe->wave_frame();
  vial::WaveFrame* loop_wave_frame = loop_frame_->wave_frame();
  ...
  compute_frame_->interpolate(keyframe, loop_frame_.get(), position / (vial::kNumOscillatorWaveFrames - 1.0f));
