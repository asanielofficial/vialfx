...
void FrequencyFilterModifier::FrequencyFilterModifierKeyframe::render(vial::WaveFrame* wave_frame) {
...
float FrequencyFilterModifier::FrequencyFilterModifierKeyframe::getMultiplier(float index) {
  ...
  float slope = 1.0f / vial::utils::interpolate(1.0f, kMaxSlopeReach, shape_ * shape_);
  float power = vial::utils::interpolate(kMinPower, kMaxPower, shape_);

  if (style_ == kLowPass)
    return vial::utils::clamp(1.0f - slope * cutoff_delta, 0.0f, 1.0f);
  if (style_ == kBandPass)
    return vial::utils::clamp(1.0f - fabsf(slope * cutoff_delta), 0.0f, 1.0f);
  if (style_ == kHighPass)
    return vial::utils::clamp(1.0f + slope * cutoff_delta, 0.0f, 1.0f);
  if (style_ == kComb)
...
void FrequencyFilterModifier::render(vial::WaveFrame* wave_frame, float position) {
