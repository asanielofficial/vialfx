...
LineGenerator::LineGenerator(int resolution) : points_(), powers_(), num_points_(2), resolution_(resolution),
  ...
  buffer_ = std::make_unique<vial::mono_float[]>(resolution + kExtraValues);
  initLinear();
}
...
void LineGenerator::render() {
  ...
  for (int i = 0; i < resolution_; ++i) {
    ...
    t = vial::utils::clamp(vial::futils::powerScale(t, current_power), 0.0f, 1.0f);
...
float LineGenerator::valueAtPhase(float phase) {
  float scaled_phase = vial::utils::clamp(phase, 0.0f, 1.0f) * resolution_;
  int index = scaled_phase;
  return vial::utils::interpolate(buffer_[index + 1], buffer_[index + 2], scaled_phase - index);
}
...
float LineGenerator::getValueBetweenPoints(float x, int index_from, int index_to) {
  ...
  t = vial::utils::clamp(vial::futils::powerScale(t, power), 0.0f, 1.0f);
