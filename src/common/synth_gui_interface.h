...
struct SynthGuiData {
  ...
  vial::control_map controls;
  vial::output_map mono_modulations;
  vial::output_map poly_modulations;
  vial::output_map modulation_sources;
  WavetableCreator* wavetable_creators[vial::kNumOscillators];
  SynthBase* synth;
};
...
class SynthGuiInterface {
  public:
    ...
    virtual void updateGuiControl(const std::string& name, vial::mono_float value);
    vial::mono_float getControlValue(const std::string& name);
